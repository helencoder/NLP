...
public class FinalSeg {
    ...
    public synchronized static FinalSeg getInstance(String prob_emitPath) {
        if (null == singleInstance) {
            singleInstance = new FinalSeg();
            singleInstance.loadModel(prob_emitPath);
        }
        return singleInstance;
    }

    private void loadModel(String prob_emitPath) {
        ...
        try {
            ...
            Path path = new Path(prob_emitPath);
            ...
            System.err.println(String.format(Locale.getDefault(), "%s: load model failure!", prob_emitPath));
        }
        finally {
            try {
                if (null != br)
                ...
                System.err.println(String.format(Locale.getDefault(), "%s: close failure!", prob_emitPath));
            }
        }
